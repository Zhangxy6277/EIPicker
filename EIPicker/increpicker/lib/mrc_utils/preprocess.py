...
def load_and_downsample(path, target_size):
    ...
    if header[2] > 1:
        ...
        avg_mrc /= header[2]
        data = avg_mrc
    data = mrc.downsample_with_size(data, int(data.shape[1]/data.shape[0]*1024), target_size)
...
def process(opt):
    ...
    if opt.data_type == 'mrc':
        ...
        downsampled_label = label_downsample(
            mrc_data, label, 
            opt.label_type, 
            opt.target_size*mrc_data[0].data.shape[1]/mrc_data[0].data.shape[0],
            opt.target_size
        )
    ...
    return get_mean_and_var(image_path, 1024*727)
