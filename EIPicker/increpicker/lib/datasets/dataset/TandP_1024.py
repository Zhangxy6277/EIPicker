...
class TandP_1024(data.Dataset):
    num_classes = 1
    default_resolution = [1024, 1024]
    mean = np.array([0.505915, 0.505915, 0.505915],
                    dtype=np.float32).reshape(1, 1, 3)
    std = np.array([0.140509, 0.140509, 0.140509],
                   dtype=np.float32).reshape(1, 1, 3)

    def __init__(self, opt, split):
        super(TandP_1024, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'TandP')
        ...
        print('==> initializing TandP {} data.'.format(split))
