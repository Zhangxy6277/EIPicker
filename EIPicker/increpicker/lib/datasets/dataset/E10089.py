...
class E10089(data.Dataset):
  num_classes = 1
  default_resolution = [1024, 1024]
  mean = np.array([0.509106, 0.509106, 0.509106],
                   dtype=np.float32).reshape(1, 1, 3)
  std  = np.array([0.288608, 0.288608, 0.288608],
                   dtype=np.float32).reshape(1, 1, 3)

  def __init__(self, opt, split):
    super(E10089, self).__init__()
    self.data_dir = os.path.join(opt.data_dir, '10089')
  ...
  def run_eval(self, results, save_dir):
    ...
    #coco_eval.params.iouThrs=np.linspace(.5, .5, int(np.round((.5- .5) / .05)) + 1, endpoint=True)
    ...
    plt.ylim(0,1.0)
    plt.grid(True)
    plt.plot(x,pr1[:,2],'b-',label='IoU=0.5')
    plt.plot(x,pr2[:,2],'c-',label='IoU=0.6')
    plt.plot(x,pr3[:,2],'y-',label='IoU=0.7')
