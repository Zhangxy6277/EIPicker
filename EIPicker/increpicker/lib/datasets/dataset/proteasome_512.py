...
class PROTEASOME_512(data.Dataset):
  ...
  mean = np.array([0.591419, 0.591419, 0.591419],
                   dtype=np.float32).reshape(1, 1, 3)
  std  = np.array([0.291782, 0.291782, 0.291782],
                   dtype=np.float32).reshape(1, 1, 3)

  def __init__(self, opt, split):
    super(PROTEASOME_512, self).__init__()
    self.data_dir = os.path.join(opt.data_dir, 'proteasome_512')
