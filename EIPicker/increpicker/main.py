...
def main(opt):
  ...
  optimizer = torch.optim.Adam(model.parameters(), opt.lr)#,weight_decay=0.1)
  start_epoch = 0
  #print(model)
  ...
  for epoch in range(start_epoch + 1, opt.num_epochs + 1):
    ...
    log_dict_train, _ = trainer.train(epoch, train_loader,old_loader,_means,precision_matrices)
    ...
    if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
      ...
      with torch.no_grad():
        log_dict_val, preds = trainer.val(epoch, val_loader,old_loader,_means,precision_matrices)
